...
${data.sos.length===0?'<p style="color:#888">None.</p>':'<table><tr><th>Type</th><th>Items</th><th>Table</th><th>Time</th><th>Status</th></tr>'+data.sos.map(s=>'<tr><td>'+s.request_type?.replace(/_/g,' ')+'</td><td>'+((s.sos_request_items||[]).map(li=>li.item_name+' x'+li.quantity).join(', ')||'-')+'</td><td>Table '+(s.tables?.table_number||'?')+'</td><td>'+new Date(s.created_at).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',hour12:true})+'</td><td><span class="badge '+(s.status==='resolved'?'g':s.status==='acknowledged'?'y':'r')+'">'+s.status+'</span></td></tr>').join('')+'</table>'}
            ...
            : data.sos.map((s,i) => (
              <div key={i} style={{ padding:'10px 0',borderBottom:'1px solid var(--line)',fontSize:13 }}>
                <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}>
                  ...
                  <span style={{ color:'var(--ink2)' }}>Table {s.tables?.table_number} · {new Date(s.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}{s.waiters?.name ? ' · ' + s.waiters.name : ''}</span>
                  <span style={{ fontWeight:700,color:s.status==='resolved'?'#16A34A':s.status==='acknowledged'?'#D97706':'#DC2626' }}>{s.status}</span>
                </div>
                {/* What the table actually asked for - a bare type tells the
                    caterer nothing useful after the event. */}
                {s.sos_request_items?.length > 0 && (
                  <div style={{ marginTop:6,display:'flex',flexWrap:'wrap',gap:6 }}>
                    {s.sos_request_items.map((li,j) => (
                      <span key={j} style={{ background:'#FFF7ED',border:'1px solid #FED7AA',
                        color:'#9A3412',borderRadius:999,padding:'2px 10px',fontSize:12,fontWeight:700 }}>
