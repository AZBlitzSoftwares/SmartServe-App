      ...
      <div style={{ width:'100%', background:'#fff', borderRadius:'24px 24px 0 0', padding:'24px 20px 40px', maxHeight:'85vh', overflowY:'auto' }}>
        ...
        {loading ? (
          <div style={{ textAlign:'center', padding:40 }}>
            ...
            <div style={{ color:'var(--ink2)', fontSize:14 }}>Fetching your orders...</div>
          </div>
        ) : orders.length === 0 ? (
          <div style={{ textAlign:'center', padding:40 }}>
            <div style={{ fontSize:48, marginBottom:16 }}>🍽️</div>
            <div style={{ fontWeight:700, fontSize:16, marginBottom:8 }}>No orders yet</div>
            <div style={{ color:'var(--ink2)', fontSize:14 }}>You haven't placed any orders so far. Browse the menu and place your first order!</div>
          </div>
        ) : (
          orders.map(order => (
            <div key={order.id} style={{ background:'var(--bg)', borderRadius:16, padding:16, marginBottom:12 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                ...
                <div style={{ background:(STATUS_COLOR[order.status]||'#999')+'20', color:STATUS_COLOR[order.status]||'#999', fontSize:12, fontWeight:700, padding:'3px 10px', borderRadius:999 }}>
